...
description: skill / slash command / task プロンプト / CLAUDE.md 節を新規作成・大幅改訂した直後、または「このskill動くか試して」「プロンプト改善して」と言われたとき、エージェントの挙動不良の原因を指示側の曖昧さに求めたいときに使う。バイアスを排した実行者に実際に動かしてもらい、実行者の自己申告と指示側メトリクスの両面で評価し、改善が頭打ちになるまで反復する。
---
