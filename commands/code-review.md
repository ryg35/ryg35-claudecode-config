...
After the report, log the run so future sessions can answer "did I review this branch already?":

```bash
BRANCH="$(git branch --show-current)"
~/.claude/bin/log-review.sh code-review "branch:${BRANCH}"
```

If CRITICAL/HIGH remained: pass `"unfixed: <count> CRITICAL, <count> HIGH"` as the third arg.

To inspect later: `/review-status grep branch:${BRANCH}`.
...
After publishing the review, log the run so future sessions can answer "did I already review PR #N?":

```bash
~/.claude/bin/log-review.sh code-review "PR#<NUMBER>" "decision:<APPROVE|REQUEST_CHANGES|BLOCK>"
```

To inspect later: `/review-status grep PR#<NUMBER>`.
